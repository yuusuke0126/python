...
SAVE_FLAG = False
plt.close('all')
with open('config/uturn_with_rear_free.yaml', 'r') as yml:
...
for i in range(len(motion_plan)):
  ...
  if motion_type == "linear":
    ...
    goal = goal * pi / 180
    v = 0.0
    w = np.sign(goal) * abs(motion_plan[i]["curve_radius"])
    step_num = np.ceil(abs(goal / (w*dt))).astype(int)
  current_t = z.shape[1]
...
fig = plt.figure()
ims = []
area_rect = np.loadtxt('config/areas/nisshin.csv', delimiter=',')
# print(area_rect)
...
for i in range(z.shape[1]):
  ...
  cargo_center_right[:,i] = (cargo_rect_angle[:,0] + cargo_rect_angle[:,1])/2.0 + z[:,i]
  kc_front_left[:,i] = kc_rect_angle[:,2] + z[:,i]  # np.array([kc_rect_angle[0,2]+z[0,i], kc_rect_angle[1,2]+z[1,i]])
  dist[i] = np.linalg.norm(cargo_rear_right[:,i] - area_rect[:,1])
