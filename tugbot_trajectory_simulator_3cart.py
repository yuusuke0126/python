...
def update_status(t, z, p, p2, p3, theta, phi, phi2, phi3, v=0.5, w=0.0):
    xd = v * cos(theta[t])
    yd = v * sin(theta[t])
    thetad = w
    ...
    p3xd = p2xd - (-p3_dist) * phi2d * sin(phi2[t])
    p3yd = p2yd + (-p3_dist) * phi2d * cos(phi2[t])
    if c3_dist == 0.0:
        phi3d = phi2d
    else:
        phi3d = (-p3xd * sin(phi3[t]) + p3yd * cos(phi3[t])) / c3_dist
    z[0, t + 1] = z[0, t] + xd * dt
    z[1, t + 1] = z[1, t] + yd * dt
    ...
    p3[0, t + 1] = p3[0, t] + p3xd * dt
    p3[1, t + 1] = p3[1, t] + p3yd * dt
...
p3_dist = config['p3_dist']  # distance between pivot point of 2nd cart and pivot point of 3rd cart
c3_dist = config['c3_dist']  # distance between pivot point of 3rd cart and 3rd cart wheel center
...
cf3_dist = config['cf3_dist']  # distance between pivot point of 3rd cart and 3rd cart front edge
...
init_phi3 = config['init_phi3'] * pi / 180  # initial 3rd cart angle
...
z[0, 0], z[1, 0], theta[0], phi[0], phi2[0], phi3[0] = x0, y0, init_theta, init_phi, init_phi2, init_phi3
p[0, 0] = z[0, 0] + (-p_dist) * cos(theta[0])
p[1, 0] = z[1, 0] + (-p_dist) * sin(theta[0])
p2[0, 0] = p[0, 0] + (-p2_dist) * cos(phi[0])
p2[1, 0] = p[1, 0] + (-p2_dist) * sin(phi[0])
p3[0, 0] = p2[0, 0] + (-p3_dist) * cos(phi2[0])
p3[1, 0] = p2[1, 0] + (-p3_dist) * sin(phi2[0])
...
for i in range(len(motion_plan)):
    ...
    update_status(t - 1, z, p, p2, p3, theta, phi, phi2, phi3, v, w)
...
if c2_dist == 0.0:
    ...
    pivot2_line = np.array([[0, -c2_dist],
                            [0, 0]])
if c3_dist == 0.0:
    pivot3_line = np.array([[0, -cf3_dist],
                            [0, 0]])
else:
    pivot3_line = np.array([[0, -c3_dist],
                            [0, 0]])
...
for i in tqdm.tqdm(range(z.shape[1])):
    ...
    if i % int(round(0.1/dt)) == 0:
      ...
      cargo3 = ax.fill(cargo3_rect_angle[0]+p3[0,i], cargo3_rect_angle[1]+p3[1,i], color='r')
      ...
      pivot3 = ax.plot(pivot3_angle[0]+p3[0,i], pivot3_angle[1]+p3[1,i], linewidth=4.0, color='k')
...
print("max angle2: %1.2f [deg]" % (max(abs(phi-phi2)*180/pi)))
print("max angle3: %1.2f [deg]" % (max(abs(phi2-phi3)*180/pi)))
